...
#
#optional arguments:
#  -h, --D:/Pro/OpenMVS/install/bin/help            show this help message and exit
...
# 
# python MvgMvs_Pipeline.py /home/badwolf/Pictures/castle /home/badwolf/Pictures/castle_out
    ...
    def __init__(self):
        self.steps_data=[
            [   "Densify point cloud",
                ...
                ["-i", "scene.mvs", "-w","%mvs_dir%", "--max-threads", "0" ]],
            [   "Reconstruct the mesh",
                os.path.join(OPENMVS_BIN,"ReconstructMesh"),
                ["-i", "scene_dense.mvs", "-w","%mvs_dir%", "-v", "2", "--max-threads", "0" ]],
            [   "Refine the mesh",
                os.path.join(OPENMVS_BIN,"RefineMesh"),
                ["-i", "scene_dense_mesh.mvs", "-w","%mvs_dir%", "-v", "2", "--max-threads", "2", "--reduce-memory", "1", "--resolution-level","3" ]],
            [   "Texture the mesh",
                os.path.join(OPENMVS_BIN,"TextureMesh"),
                ["-i", "scene_dense_mesh_refine.mvs", "-w","%mvs_dir%", "-v", "2", "--max-threads", "0" ]]
            ]
